      ...
      <div className="w-full max-w-md h-full bg-neutral-950 border-l border-neutral-800 flex flex-col justify-between shadow-2xl">
        ...
        <div className="p-4 border-b border-neutral-800 flex items-center justify-between bg-neutral-900/80">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h3 className="font-sans font-semibold text-sm text-neutral-100">
                {language === 'pt' ? 'Artigos Salvos' : 'Saved Articles'} ({savedArticles.length})
              </h3>
              <span className="font-mono text-[10px] text-neutral-400 block">
                {language === 'pt' ? 'Sua biblioteca pessoal' : 'Your personal library'}
              </span>
            </div>
          </div>
          <button 
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3 font-sans text-xs">
          {savedArticles.map((article) => (
            ...
            >
              <a href={`/artigo/${article.slug}`} className="space-y-1 cursor-pointer flex-1">
          ...
          {savedArticles.length === 0 && (
            <div className="py-16 text-center space-y-2">
              ...
              <p className="text-neutral-500 font-mono text-xs">
                {language === 'pt' ? 'Nenhum artigo salvo ainda.' : 'No saved articles yet.'}
              </p>
            </div>
          )}
        </div>
